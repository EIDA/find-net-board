            ...
            if (r["name"] == "archive" or r["name"] == "ARCHIVE") and "datasets" in r:
                ...
                for s in r["services"]:
                    if s["name"] == "fdsnws-station-1":
                        station_url = s["url"]
        datacenters_urls[urlparse(station_url).netloc] = dc["name"]
...
def update_datacite_table():
    pass

def try_EIDA_routing():
    pass

def try_FDSN_routing():
    pass

def update_stationxml_table():
    pass
...
def process_networks():
    ...
    with alive_bar(len(r.json()["networks"])) as pbar:
        for net in r.json()["networks"]:
            ...
            # routing table
            datacenter = None
            end = datetime.strptime("2100-01-01", "%Y-%m-%d") if end is None else end
            ...
            for dc in eida_routing:
                for network in dc["params"]:
                    ...
                    if network["net"] == net["fdsn_code"] and start <= net_start <= end:
                        datacenter = datacenters_urls[urlparse(dc["url"]).netloc]
                        break
                if datacenter:
                    ...
                    break
            # if no datacenter found try FDSN routing
            if not datacenter:
